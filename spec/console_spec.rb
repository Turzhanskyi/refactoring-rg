  ...
  describe '#call' do
    context 'when correct method calling' do
      ...
      it 'create account if input is create' do
        allow(current_subject).to receive_message_chain(:gets, :chomp) { I18n.t(:create, scope: :COMMANDS) }
        expect(current_subject).to receive(:create)
      end

      it 'load account if input is load' do
        allow(current_subject).to receive_message_chain(:gets, :chomp) { I18n.t(:load, scope: :COMMANDS) }
